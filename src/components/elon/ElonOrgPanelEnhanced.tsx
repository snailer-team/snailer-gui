...
function triadLabel(role?: TriadRole): string {
  if (!role) return ''
  const labels: Record<TriadRole, string> = {
    pm: 'PM',
    designer: 'Design',
    engineer: 'Eng',
    lead: 'Lead',
      ...
      <div className="flex items-center gap-2">
        ...
        <div className="min-w-0 flex-1">
          <div className="flex items-center gap-1.5">
            {agent.triadRole && (
              <span className="shrink-0 text-[10px]" title={triadLabel(agent.triadRole)}>
                {triadIcon(agent.triadRole)}
              </span>
            )}
            <span className="truncate text-[11px] font-semibold text-black/80 max-w-[90px]">{agent.title.split('/')[0].split('Agent')[0].trim()}</span>
...
export function ElonOrgPanelEnhanced() {
  ...
  return (
    <div className="h-full flex flex-col rounded-2xl border border-black/10 bg-white/50">
      ...
      <div className="shrink-0 flex items-center justify-between px-4 py-3 border-b border-black/5">
        <span className="text-sm font-semibold text-black/80">Org</span>
      ...
      <div className="flex-1 min-h-0 grid grid-rows-[1fr_auto]">
        ...
        {selected && (
          <div className="shrink-0 border-t border-black/5 p-3 bg-white/30 max-h-[280px] overflow-y-auto">
            ...
            {/* ReAct Loop */}
            <div className="mb-2">
              <div className="text-[9px] font-semibold text-black/40 mb-1">LOOP</div>
              <div className="flex flex-wrap gap-1">
                {selected.loop.map((step, i) => (
                  <span
                    key={i}
                    className="rounded-full border border-black/10 bg-black/5 px-2 py-0.5 text-[9px] text-black/60"
                  >
                    {step.split(':')[0]}
                  </span>
                ))}
